...
1. Добавьте бота в группу и разрешите ему отправлять сообщения.
2. Со своего аккаунта организатора (`ADMIN_IDS`) отправьте в группе
...
3. В личном чате откройте зафиксированный расчёт, нажмите **Опубликовать итог в группе**
...
Привязать группу может только организатор из `ADMIN_IDS`. Обычные сообщения в группе
игнорируются, отключать privacy mode не нужно. Права администратора боту не требуются,
если группе разрешено принимать его сообщения.
Для новой группы повторите `/bind` там: уже опубликованные тренировки продолжат
обновляться в исходной группе. До нажатия кнопки публикации финансовые данные
...
- Управление разрешено только `ADMIN_IDS` в личных чатах. В группе доступна только
  привязка командой `/bind`; итог публикуется по кнопке организатора из личного чата.
